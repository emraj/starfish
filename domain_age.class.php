...
class domain_age{
  ...
  "sc"                =>  array("whois2.afilias-grs.net","/Creationated On:(.*)/"),
  ...
  global $daysOnly;
  $daysOnly = floor($time / 86400); // Convert to a uniform unit (days) to allow for simpler calculations.
  if($years == "1")
    {$y= "1 year";}
  else {$y = $years . " years";}
  if($days == "1")
    {$d = "1 day";}
  else {$d = $days . " days old";}
  echo "$y, $d. <p> Age in days only: $daysOnly</p>";
  return $daysOnly;
  }
  else
  return false;
